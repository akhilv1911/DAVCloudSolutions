...
def home():
  ...
  # Fetch approved reviews to dynamically populate the home page marquee
...
def submit_review():
  ...
  review_doc = {
      ...
      "rating": rating,
      "feedback": feedback,
      "status": "approved",  # Approved for instant display in marquee
...
def business_catalog():
  """Verified Business Directory Page - Shows real completed client deliverables without low price anchoring."""
  real_projects = [
      {
      ...
      },
      {
          ...
          "description": (
              ...
              " seamless course delivery and student engagement."
          ),
          "tech_stack": [
              "Python Architecture",
              "Streamlit",
...
def handle_inquiry():
  ...
  if (
    ...
    return jsonify({
        "success": True,
        "message": (
            "Your inquiry has been successfully received by Founder V Akhil."
        ),
    }), 200

  flash(
      "Your inquiry has been submitted! Founder V Akhil will reach out"
      " shortly.",
      "success",
  )
...
def register():
  ...
  if request.method == "POST":
    ...
    send_admin_inquiry_notification({
        ...
        "message": (
            f"A new {role} registered ({email}). Awaiting email confirmation"
            " and Admin approval."
        ),
    })

    flash(
        "Registration successful! Please check your email inbox to verify your"
        " email address.",
        "success",
    )
  ...
  if request.method == "POST":
    ...
    if user and check_password(user["password_hash"], password):
      ...
      if not user.get("is_admin_approved", False):
        flash(
            "Your email is verified! Your account is currently in queue for"
            " review and activation by Admin V Akhil.",
            "info",
        )
        return redirect(url_for("login"))
...
def resend_verification():
  ...
  if user:
    if user.get("is_email_verified", user.get("is_verified", False)):
      flash(
          "Your email is already verified. If your account is pending approval,"
...
def student_dashboard():
  ...
  if request.method == "POST":
    ...
    flash(
        "Project proposal submitted successfully! Founder V Akhil will review"
        " the scope.",
        "success",
    )
    return redirect(url_for("student_dashboard"))
...
def business_dashboard():
  ...
  if request.method == "POST":
    ...
    flash(
        "Business MVP requirement submitted successfully! We will prepare a"
